...
with tab_overview:
    ...
    total_profit = int(df_filtered['Profit'].sum()) if not df_filtered['Profit'].isna().all() else 0
    ...
    c1.metric("💰 Total Sell", f"{total_sell:,}")
...
with tab_customers:
    ...
    if 'Customer Phone' in df_filtered.columns and 'Order Date' in df_filtered.columns:
        rfm = df_filtered.groupby('Customer Phone').agg(frequency=('Order Date','count'),
                                                       monetary=('Sell','sum'),
                                                       last_order=('Order Date','max')).reset_index()
